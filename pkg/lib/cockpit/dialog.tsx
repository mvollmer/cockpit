...
/* This is a framework for conveniently implementing dialogs. It is
   ...
   A DialogActionButton has a number of extra features:

     - You can specify a "actionVariant", which will be passed to the
       action function as the second argument.  The variant defaults
       to "main". The "main" button will be rendered as a primary
       button, and any other variant will be rendered as a secondary
       button.

     - You can specify a "excuse".  Buttons with a excuse are always
       disabled and show the excuse in a tooltip.
...
export function DialogActionButton<V>({
    ...
    const sfx = actionVariant == "main" ? "" : "-" + actionVariant;
    const id = `dialog-apply${sfx}`;
    ...
    const btn = (
        <Button
            ...
            variant={actionVariant == "main" ? "primary" : "secondary"}
            onClick={async () => {
                cockpit.assert(dialog && !(dialog instanceof DialogError));
                setRunning(true);
                if (await dialog.run_action(v => action(v, actionVariant)) && onClose)
                    onClose();
                setRunning(false);
            }}
            {...props}
