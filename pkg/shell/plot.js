...
shell.plot = function plot(element, x_range_seconds, x_stop_seconds) {
    ...
    function add_monitor(monitor, opts) {
        ...
        function reset_series() {
            ...
            filler = monitor.fill(series_data, plot_start, plot_end, interval, refresh);
 ...
 *
 * - promise = monitor.fill(array, start, end, interval, notify)
 *
 ...
 *
 * The NOTIFY function will be called whenever ARRAY has been filled a
 * bit more.
 *
 * XXX - document promise.cancel()
 *
 * - promise = monitor.has_archives()
 *
 * - val = monitor.sample_real_time(start, end)
 *
 * XXX - implement with fill.
 *
 * - monitor.destroy()
 */
...
shell.metrics_sum_monitor = function metrics_sum_monitor(desc) {
    ...
    function fill(series_data, plot_start, plot_end, interval, notify) {
