    ...
    function menuitem(action) {
        const run = event => {
            if (!client.permission.allowed)
                return;
            var prom = client.run(action.func);
            if (prom)
        ...
        return <MenuItem key={action.title} onClick={run} disabled={!client.permission.allowed}>{action.title}</MenuItem>;
    }

    var kebab = null;
    if (tabs.menu_actions)
        kebab = <DropdownKebab id={"menu-" + name}>{tabs.menu_actions.map(menuitem)}</DropdownKebab>;

    var actions = <>{tabs.actions}{kebab}</>;
