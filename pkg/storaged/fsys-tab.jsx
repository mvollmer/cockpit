...
function get_fstab_config(block) {
    const config = utils.array_find(block.Configuration, function (c) { return c[0] == "fstab" });
    if (config) {
        let dir = utils.decode_filename(config[1].dir.v);
...
export function check_mismounted_fsys(client, path, enter_warning) {
    ...
    var split_options = parse_options(opts);
    ...
    if (dir) {
        ...
        let type;
        if (!is_mounted && !opt_noauto && other_mounts.length > 0)
...
function reload_systemd() {
    return cockpit.spawn(["systemctl", "daemon-reload"], { superuser: "require" });
}
...
export function mounting_dialog(client, block, mode) {
    ...
    function maybe_update_config(new_dir, new_opts) {
        var new_config = null;
        var all_new_opts = new_opts;

        if (old_config) {
            if (all_new_opts && old_parents)
        ...
        return maybe_unmount().then(() => {
            if (!old_config && new_config)
                return (block.AddConfigurationItem(new_config, {})
                        .then(maybe_mount)
                        .then(reload_systemd));
            else if (old_config && !new_config)
                return block.RemoveConfigurationItem(old_config, {}).then(reload_systemd);
            else if (old_config && new_config && (new_dir != old_dir || new_opts != old_opts))
...
export class FilesystemTab extends React.Component {
    ...
    render() {
        ...
        var [old_config, old_dir, old_opts, old_parents] = get_fstab_config(block);
        var split_options = parse_options(old_opts);
        ...
        if (!is_filesystem_mounted) {
            if (!old_dir)
                extra_text = _("The filesystem needs to be mounted in order to access the files on it.");
            else
        ...
        function fix_config() {
            ...
            const new_config = [
                "fstab", {
                    ...
                    dir: { t: 'ay', v: utils.encode_filename(type == "change-mount-on-boot" ? other : old_dir) },
                    type: { t: 'ay', v: utils.encode_filename("auto") },
                    opts: { t: 'ay', v: utils.encode_filename(all_new_opts || "defaults") },
                    freq: { t: 'i', v: 0 },
                    passno: { t: 'i', v: 0 }
        ...
        if (mismounted_fsys_warning) {
            ...
            if (type == "change-mount-on-boot") {
                ...
                text = _("The filesystem is currently mounted but will be not be mounted after the next boot.");
