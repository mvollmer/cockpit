...
class SshPeer(Peer):
    ...
    def __init__(self, router: Router, host: str, user: Optional[str], password: Optional[str], *, init_superuser: Optional[str], private: bool):
        hexuser = ''.join(f'{c:02x}' for c in user.encode('ascii'))
        super().__init__(router, auth_challenge_responses={f'plain1:{hexuser}': password})
        self.host = host
        self.user = user
